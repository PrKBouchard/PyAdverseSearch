...
class Game:
    def __init__(self):
        ...
        # positions de départ (coins)
        self.board[0][0] = 'BLEU'
        self.board[0][SIZE-1] = 'ROUGE'
        self.board[SIZE-1][0] = 'JAUNE'
        self.board[SIZE-1][SIZE-1] = 'VIOLET'

        self.current_player = 'BLEU'
        self.human = 'BLEU'
        

    def next_player(self):
        idx = PLAYERS.index(self.current_player)
        self.current_player = PLAYERS[(idx + 1) % 4]
    ...
    def get_moves(self, player):
        ...
        for r in range(SIZE):
            for c in range(SIZE):
                if self.board[r][c] == ' ' and self.is_adjacent(r,c,player):
                    moves.append((r,c))
        return moves
...
def evaluate(game, move, player):
    r, c = move
    score = 0

    for dr, dc in DIRECTIONS:
        nr, nc = r+dr, c+dc
        if game.inside(nr,nc):
            if game.board[nr][nc] == ' ':
                score += 2
            elif game.board[nr][nc] != player:
                score -= 1
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Jeu de territoire 4 joueurs")
    ...
    def draw(self):
        ...
        # afficher coups possibles humain
        if self.game.current_player == self.game.human:
            moves = self.game.get_moves(self.game.human)
    ...
    def click(self, event):
        ...
        if (r,c) in self.game.get_moves(self.game.human):
    ...
    def loop(self):
        if not self.game.is_game_over():
            if self.game.current_player != self.game.human:
                move = ai_move(self.game, self.game.current_player)
